...
export class PuzzleMode implements Mode {
  ...
  private renderHud() {
    ...
    (document.getElementById("pz-skip") as HTMLButtonElement).onclick = () => { this.streak = 0; this.next(); };
  }
  ...
  private async onMove(m: Move) {
    ...
    if (ok) {
      ...
      if (this.step >= this.solution.length) {
        ...
        sound.great();
        ui.say(`${pick(CHEERS)} ${this.s.chess.isCheckmate() ? "CHECKMATE!" : ""} ⭐`, [{ label: "Next puzzle ➜", onClick: () => this.next() }]);
        this.renderHud();
        this.s.locked = true; this.s.board.interactive = false;
        await this.opts.onSolved?.(this.solved);
        if (this.active && this.opts.rush) { await wait(0.9); this.next(); }
        return;
      }
      // play the reply
      this.s.locked = true;
      ...
      // explain with the engine's best reply to the tried move
      let reply: Move | null = null;
      try {
        const ev = await this.ctx.engine.evaluate(m.after, { depth: 10 });
        if (ev.bestMove) reply = moveFromUci(new Chess(m.after), ev.bestMove);
      } catch { /* engine busy */ }
      ...
      await this.s.undo();
      this.renderHud();
      if (this.opts.rush && this.lives <= 0) { await wait(1.0); this.finish(); return; }
